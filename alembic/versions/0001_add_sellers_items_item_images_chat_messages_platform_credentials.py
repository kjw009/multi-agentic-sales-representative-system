...
item_condition = PgEnum(
    "new", "like_new", "good", "fair", "poor", name="item_condition", create_type=False
)
item_status = PgEnum(
    "pending",
    "intake_in_progress",
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE TYPE item_status AS ENUM ('pending', 'intake_in_progress', 'intake_complete', 'priced', 'publishing', 'live', 'sold', 'removed', 'error')"
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "seller_id", "platform", name="uq_platform_credentials_seller_platform"
        ),
    )
